...
export class FilesService implements FilesRepository {
  ...
  public async getFilesByEventId(eventId: string): Promise<File[]> {
    this.logger.debug(`Executing query: getFilesByEventId (${eventId})`);
    const query = `SELECT f.id, f.name, f.path, f.format, f.hash, f.size, f.user_creator
                   FROM config.files f
                            INNER JOIN config.files_events fe ON f.id = fe.file_id
                   WHERE fe.event_id = $1
                     AND fe.is_delete = false
                     AND f.is_deleted = false`;
    try {
      const result = await this.dbService.execute(query, [eventId]);
      return result.rows.map((row) => File.parseQuery(row));
    } catch (error) {
      this.logger.error(
        `Error executing query getFilesByEventId (${eventId}), error: ${error}`,
      );
      return [];
    }
  }
